    ...
    echo "
                    ...
                    <div style='width:100%;margin:0 auto;background-color:#4f81b3;overflow:hidden'>
                        ...
                        <div class='footer'>
                            ...
                            <div class='left'>Copyright © " . date(Y) . " Duy The.</div>
...
//$data=$data_date[1].'-'.$data_date[0].'-'.$data_date[2];
  ...
  function generateForms_main($arrayConst) {
    ...
    echo "<div id='panelForm' class='panelForm'>
				...
				<table id='mainForm' class='mainForm' cellpadding='1' cellspacing='1'>";
    echo $this->returnTitleMenu($arrayConst["titlePage"]);
    echo "<tr><td class='boxGrey2'  colspan='2'>" . (($isInsert) ? $this->stateInsert() : (($isEdit) ? $this->stateUpdate() : '')) . "</td></tr>";
    echo "<tr><td colspan='4' class='txtdo'>$msg</td></tr>";
    echo $option;
            ...
            case "int" :
              if ($field['Length'] <= 4) {
                ...
                if ($field['Length'] == 20 && strstr(strtolower($Field), "date") != "") {
                  ...
                  echo "<tr><td class='boxGrey'>" . $Label . "</td><td class='boxGrey2'>" . $html->input($Field, array('class' => 'nd', 'type' => 'text', 'value' => $arrayValue[$Field], 'onfocus' => 'fo(this)', 'onblur' => 'lo(this)', 'maxlength' => '12', 'onkeypress' => "return nhapso(event,'" . $Field . "')")) . "</td></tr>";
              }
              break;
            case "real" :
              if ($arrayValue[$Field] == "")
          ...
          if (!empty($excludeCol[2][$value])) {
            ...
            switch ($dataType) {
              ...
              case "bigint" :
                ...
                elseif ($dataLength == 20 && strstr(strtolower($value), "date") != "") {
...
// lay cap 4
  ...
  function link_option($Param) {
    $row = $Param['row'];
    $str = '<a onclick="openBox(\'viewhinh.php?id=' . $row["id"] . '\',900,700);" href="javascript:void(0)" style="width:70px; float:left; color:red"><b>Xem hình</b></a>';
    return $str;
  }

  function link_detail_tour($Param) {
    $row = $Param['row'];
    $str = '<a href="chiphitour_detail.php?id=' . $row["id"] . '" style="width:70px; float:left; color:red"><b>Chi tiết</b></a>';
    return $str;
  }
